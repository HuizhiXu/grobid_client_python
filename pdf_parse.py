...
class Parse:
    ...
    def list_pdf_paths(self):
        """
        list of pdf paths in pdf folder
        """
        return glob(os.path.join(self.pdf_folder, "*.pdf"))
    ...
    def parse_pdf(
        ...
        if isinstance(pdf_path, str):
            if (
                self.validate_url(pdf_path)
                and os.path.splitext(pdf_path)[-1].lower() != ".pdf"
            ):
                print("The input URL has to end with ``.pdf``")
                parsed_article = None
            elif (
                self.validate_url(pdf_path) and os.path.splitext(pdf_path)[-1] == ".pdf"
            ):
        ...
        if soup and parsed_article:
            parsed_article = BeautifulSoup(parsed_article, "lxml")
        return parsed_article

    def calculate_number_of_references(self, div):
        """
    ...
    def parse_sections(self, article, as_list: bool = False):
        """
        ...
        for div in divs:
            ...
            if heading != "" or text != "":
                ref_dict = self.calculate_number_of_references(div)
                sections.append(
                    {
                        "heading": heading,
                        "text": text,
    ...
    def parse_figure_caption(self, article):
        """
    ...
    def parse_formulas(self, article):
        """
    ...
    def convert_article_soup_to_dict(self, article, as_list: bool = False):
        """
        ...
        if article is not None:
            ...
            article_dict["authors"] = self.parse_authors(article)
            article_dict["pub_date"] = self.parse_date(article)
            article_dict["abstract"] = self.parse_abstract(article)
            article_dict["sections"] = self.parse_sections(article, as_list=as_list)
            article_dict["references"] = self.parse_references(article)
            article_dict["figures"] = self.parse_figure_caption(article)
            article_dict["formulas"] = self.parse_formulas(article)
    ...
    def parse_pdf_to_dict(
        ...
        article_dict = self.convert_article_soup_to_dict(
...
if __name__ == "__main__":
    ...
    pdf_folder = "data/grobid_data"
    parse_class = Parse(pdf_folder)
    parse_class.list_pdf_paths()

    spdf_path = "data/grobid_data/catal9121069.pdf"
    article = parse_class.parse_pdf(pdf_path=spdf_path, fulltext=True, soup=True)
    print(article)

    pdf_dict = parse_class.convert_article_soup_to_dict(article=article)
    print(pdf_dict)
